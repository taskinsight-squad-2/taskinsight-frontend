...
function slaInfo(
  ...
  const ms =
    new Date(finished).getTime() - new Date(originalDeadline).getTime();
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    setTaskStatuses(
      Object.fromEntries(apiTasks.map((t) => [t._id, mapStatus(t.status)])),
    );
    setTaskDates(
      Object.fromEntries(
        apiTasks.map((t) => [
  ...
  const [deadlineChangeStep, setDeadlineChangeStep] = useState<
    "ask" | "edit" | null
  >(null);
  ...
  async function confirmStart(id: string) {
    ...
    await taskService
      .update(id, { status: "IN_PROGRESS" }, token)
      .catch(() => {});
    await fetchTasks();
  }
  async function finishTask(id: string) {
    ...
    if (!dates?.started) {
      showToast("⚠ Inicie a tarefa antes de concluí-la.");
      return;
    }
    setTaskStatuses((prev) => ({ ...prev, [id]: "Done" }));
    setTaskDates((prev) => ({
      ...prev,
      [id]: { ...prev[id], finished: today() },
    }));
  ...
  async function confirmDeadlineChange(id: string) {
    ...
    await taskService
      .update(
        id,
        { dueDate: isoVal, deadlineChangeReason: deadlineChangeReason.trim() },
        token,
      )
      .catch(() => {});
  ...
  function startEdit(id: string) {
    setDraftDesc((prev) => ({ ...prev, [id]: descriptions[id] ?? "" }));
    setEditing((prev) => ({ ...prev, [id]: true }));
  }
  function saveDesc(id: string) {
    setDescriptions((prev) => ({ ...prev, [id]: draftDesc[id] ?? "" }));
  ...
  function completeAll() {
    setTaskStatuses((prev) =>
      Object.fromEntries(Object.keys(prev).map((k) => [k, "Done" as Status])),
    );
  }
  function clearDone() {
    setTaskStatuses((prev) =>
      Object.fromEntries(Object.entries(prev).filter(([, v]) => v !== "Done")),
    );
  }
  ...
  const tasks = useMemo(
    () =>
      apiTasks.map((apiTask) => ({
        ...
        status: (taskStatuses[apiTask._id] ??
          mapStatus(apiTask.status)) as Status,
      })),
    [apiTasks, taskStatuses],
  );
  const [priorityFilter, setPriorityFilter] = useState<Priority | "All">("All");
  function setPriorityFilterReset(p: Priority | "All") {
    setPriorityFilter(p);
    setPage(1);
  }
    ...
    () =>
      tasks.filter(
        (t) =>
          ...
          taskDates[t.id]?.deadline &&
          taskDates[t.id].deadline! < today(),
      ).length,
    [tasks, taskDates],
  );
  ...
  function setFilterReset(f: FilterTab) {
    setFilter(f);
    setPage(1);
  }
  function setPageSizeReset(n: number) {
    setPageSize(n);
    setPage(1);
  }
  function setSortReset(s: SortMode) {
    setSortMode(s);
    setPage(1);
  }
  ...
  const pagBtnActive = dark
    ? "bg-violet-600 border-violet-600 text-white"
    : "bg-violet-600 border-violet-600 text-white";
  const pagBtnInact = dark
  ...
  const availableYears = useMemo(() => {
    ...
    const taskYears = new Set(
      apiTasks
        .map((t) => Number(t.createdAt?.slice(0, 4)))
        .filter((y) => y >= 2026),
    );
  ...
  const chartData = useMemo(() => {
    ...
    const planned = points.map((_, i) =>
      Math.round((total / Math.max(1, n - 1)) * i),
    );
    const done = points.map(
      (date) =>
        apiTasks.filter((t) => {
          const isDone = taskStatuses[t._id] === "Done";
          const finished =
            taskDates[t._id]?.finished ?? (isDone ? today() : null);
  ...
  function gx(i: number) {
    return GP + i * ((GW - GP * 2) / Math.max(1, nPoints - 1));
  }
  function gy(v: number) {
    return (
      GH - GP - Math.round(((v - 1) / Math.max(1, gMax - 1)) * (GH - GP * 2))
    );
  }
  ...
  const areaDeviation =
    `${pathDone} L ${gx(nPoints - 1)} ${gy(plannedSeries[nPoints - 1])} ` +
    [...plannedSeries]
      .reverse()
      .map((v, i) => {
  ...
  const sidebar = (
    ...
    >
      {/* header com toggle */}
      <div
        className={`flex items-center border-b ${sidebarBdr} transition-all duration-300 ${collapsed ? "justify-center px-0 py-5" : "gap-2.5 px-5 py-5"}`}
      >
        {!collapsed && (
        ...
        {!collapsed && (
          <span className={`font-bold ${text} text-base tracking-tight flex-1`}>
            TaskFlow
          </span>
        )}
        <button
      ...
      <nav
        ...
        className="flex flex-col gap-0.5 px-2 py-4 flex-1 overflow-y-auto overflow-x-hidden"
      >
        {!collapsed && (
          <p
            className={`text-[10px] font-bold ${sectionLbl} uppercase tracking-[0.15em] px-3 mb-2`}
          >
            {t.menu}
          </p>
        )}
        ...
        {!collapsed && (
          <>
            <div className="flex items-center px-3 mt-6 mb-2">
              <p
                className={`text-[10px] font-bold ${sectionLbl} uppercase tracking-[0.15em]`}
              >
                {t.categories}
              </p>
            </div>
            ...
            >
              <span className="flex items-center gap-2">
                <span
                  className={`w-2 h-2 rounded-full flex-shrink-0 ${dark ? "bg-white/30" : "bg-slate-400"}`}
                />
                Todas
              </span>
              <span
                className={`text-[11px] border rounded-full px-2 py-0.5 font-mono ${priorityFilter === "All" ? (dark ? "bg-violet-500/20 border-violet-500/30 text-violet-300" : "bg-violet-100 border-violet-200 text-violet-600") : catBadge}`}
              >
                {tasks.length}
              </span>
            </button>
            {categories.map((cat) => {
              ...
              return (
                <button
                  key={cat.label}
                  onClick={() =>
                    setPriorityFilterReset(isActive ? "All" : cat.priority)
                  }
                ...
                >
                  ...
                  <span
                    className={`text-[11px] border rounded-full px-2 py-0.5 font-mono ${isActive ? (dark ? "bg-violet-500/20 border-violet-500/30 text-violet-300" : "bg-violet-100 border-violet-200 text-violet-600") : `${catBadge}`}`}
                  >
                    {cat.count}
                  </span>
                </button>
      ...
      {/* footer */}
      <div
        className={`border-t ${sidebarBdr} flex items-center transition-all duration-300 ${collapsed ? "justify-center px-2 py-4" : "gap-3 px-4 py-4"}`}
      >
  ...
  return (
    <div
      className={`min-h-screen flex ${pageBg} ${text} transition-colors duration-300`}
    >
      {/* modal nova tarefa */}
      {showNewTask && (
        <div
        ...
        >
          ...
          <div
            ref={trapNewTask}
            className={`relative w-full max-w-md mx-4 ${dark ? "bg-[#0D1117] border-white/10" : "bg-white border-slate-200"} border rounded-2xl p-6 shadow-2xl`}
          >
            <h2
              id="modal-new-task-title"
              className={`text-base font-bold ${dark ? "text-white" : "text-slate-900"} mb-5`}
            >
              Nova Tarefa
            </h2>
            <form
            ...
            >
              <div>
                <label
                  ...
                  className={`text-xs font-semibold ${dark ? "text-white/65" : "text-slate-600"} uppercase tracking-widest mb-1.5 block`}
                >
                  Título
                </label>
                <input
              ...
              <div>
                ...
                <textarea
                  ...
                  className={
                    dark
                      ? "w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-white/40 outline-none focus:border-violet-500/60 resize-none transition"
                      : "w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 placeholder-slate-400 outline-none focus:border-violet-400 resize-none transition"
                  }
                />
              </div>
              <div>
      ...
      {confirmDeadlineId &&
        (() => {
          const taskTitle =
            tasks.find((t) => t.id === confirmDeadlineId)?.title ?? "";
          ...
          const closeModal = () => {
            ...
            setDeadlineChangeStep(null);
            setDeadlineChangeReason("");
            setDeadlineDraft((prev) => ({ ...prev, [confirmDeadlineId]: "" }));
          };
          return (
            <div
              className="fixed inset-0 z-50 flex items-center justify-center"
              role="dialog"
              aria-modal="true"
              aria-labelledby="modal-deadline-title"
            >
              <div
                className="absolute inset-0 bg-black/50 backdrop-blur-sm"
                onClick={closeModal}
              ...
              <div
                ref={trapDeadline}
                className={`relative w-full max-w-sm mx-4 ${dark ? "bg-[#0D1117] border-white/10" : "bg-white border-slate-200"} border rounded-2xl p-6 shadow-2xl`}
              >
                <div className="flex items-center gap-3 mb-3">
                  <div
                    aria-hidden="true"
                    className="w-9 h-9 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-500 flex-shrink-0"
                  >
                    <svg
                      width="16"
                      height="16"
                ...
                <p
                  className={`text-xs ${dark ? "text-white/55" : "text-slate-500"} mb-4`}
                >
                  <span
                    className={`font-semibold ${dark ? "text-white/80" : "text-slate-700"}`}
                  >
                    {taskTitle}
                  </span>
                </p>
                ...
                {deadlineChangeStep === "ask" && (
                  <div className="flex gap-2 justify-end">
                    ...
                    <button
                      onClick={() => setDeadlineChangeStep("edit")}
                ...
                {deadlineChangeStep === "edit" && (
                  <>
                    <div className="mb-3">
                      <label
                        className={`text-xs font-semibold ${dark ? "text-white/50" : "text-slate-500"} uppercase tracking-widest mb-1.5 block`}
                      >
                        Nova data
                      </label>
                      <input
                        ...
                        onChange={(e) =>
                          setDeadlineDraft((prev) => ({
                            ...prev,
                            [confirmDeadlineId]: e.target.value,
                          }))
                        }
                        className={
                          dark
                            ? "w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white outline-none focus:border-amber-500/50 transition"
                            : "w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-800 outline-none focus:border-amber-400 transition"
                        }
                      />
                    </div>
                    <div className="mb-4">
                      ...
                      <textarea
                        rows={2}
                        placeholder="Ex: cliente solicitou extensão do prazo..."
                        value={deadlineChangeReason}
                        onChange={(e) =>
                          setDeadlineChangeReason(e.target.value)
                        }
                        className={
                          dark
                            ? "w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder-white/30 outline-none focus:border-amber-500/50 resize-none transition"
                            : "w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-800 placeholder-slate-400 outline-none focus:border-amber-400 resize-none transition"
                        }
                      />
                    </div>
                    <div className="flex gap-2 justify-end">
                      ...
                      <button
                        onClick={() => confirmDeadlineChange(confirmDeadlineId)}
                        disabled={
                          !deadlineChangeReason.trim() ||
                          !deadlineDraft[confirmDeadlineId]
                        }
            ...
            >
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  <div
                    aria-hidden="true"
                    className="w-9 h-9 rounded-xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-blue-500 flex-shrink-0"
                  >
                    <svg
                      width="16"
                      height="16"
                ...
                <p
                  className={`text-xs ${dark ? "text-white/55" : "text-slate-500"} mb-1`}
                >
                  <span
                    className={`font-semibold ${dark ? "text-white/80" : "text-slate-700"}`}
                  >
                ...
                <p
                  className={`text-xs ${dark ? "text-white/40" : "text-slate-400"} mb-5`}
                >
                  Ao iniciar, o tempo começa a ser contado. Certifique-se de que
                  vai trabalhar nela agora.
                </p>
                <div className="flex gap-2 justify-end">
                  <button
                    onClick={() => setConfirmStartId(null)}
                  ...
                  <button
                    onClick={() => confirmStart(confirmStartId)}
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-40 flex md:hidden">
          <div
            className={`fixed inset-0 ${mobileOverlay} backdrop-blur-sm`}
      ...
      >
        {/* Header */}
        <div
          className={`flex items-center justify-between px-6 py-4 border-b ${headerBdr} ${headerBg} backdrop-blur-xl sticky top-0 z-30 transition-colors duration-300`}
        >
          <div className="flex items-center gap-3">
            <button
              ...
              className={`md:hidden ${mobileTrigger} transition`}
            >
              <svg
                aria-hidden="true"
                width="20"
            ...
            <div>
              <h1 className={`text-base font-bold ${text} tracking-tight`}>
                {greeting}
                {userName ? `, ${userName}` : ""} 👋
              </h1>
              <p className="text-xs text-violet-500 mt-0.5">
                {t.pendingTasksMsg(nPending)}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => setPrefs("darkMode", !dark)}
            ...
            <button
              ...
              aria-label="Criar nova tarefa"
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 text-white text-xs font-semibold hover:from-violet-500 hover:to-indigo-500 transition shadow-lg shadow-violet-500/25"
            >
              <svg
                aria-hidden="true"
                width="12"
        ...
        <div className="px-6 py-6 flex flex-col gap-5">
          {loadingTasks && (
            ...
            >
              <span
                aria-hidden="true"
                className="w-6 h-6 border-2 border-violet-500/30 border-t-violet-500 rounded-full animate-spin"
              />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <div
                key={s.label}
                className={`${cardBg} border rounded-xl px-4 py-3 flex items-center gap-3 transition-colors`}
              >
                <div
                  className={`w-7 h-7 rounded-lg ${s.iconBg} border flex items-center justify-center ${s.accent} flex-shrink-0`}
                >
                  {s.icon}
                </div>
                <div>
                  <p
                    className={`text-[10px] font-semibold ${textFaint} uppercase tracking-widest`}
                  >
                    {s.label}
                  </p>
                  <p className={`text-xl font-black ${text} leading-tight`}>
                    {s.value}
                  </p>
                </div>
              </div>
          ...
          {/* progress bar */}
          <div
            className={`${cardBg} border rounded-xl px-4 py-3 flex items-center gap-3`}
          >
            <span
              className={`text-xs font-semibold ${textMuted} whitespace-nowrap`}
              aria-hidden="true"
            >
              Conclusão
            </span>
            ...
            <span
              className={`text-xs font-bold ${text} tabular-nums w-8 text-right`}
              aria-hidden="true"
            >
              {completion}%
            </span>
          ...
          <div className={`${cardBg} border rounded-2xl overflow-hidden`}>
            {/* toolbar */}
            <div
              className={`flex items-center justify-between px-5 py-3 border-b ${listBdr} flex-wrap gap-2`}
            >
                ...
                {filterTabs.map((f) => (
                  ...
                  >
                    ...
                    {f.badge !== undefined && f.badge > 0 && (
                      <span
                        className={`text-[10px] font-bold rounded-full px-1.5 py-px leading-none ${filter === f.key ? "bg-white/25 text-white" : "bg-red-500 text-white"}`}
                      >
                        {f.badge}
                      </span>
                    )}
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={completeAll}
                  className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 hover:bg-emerald-500/20 transition"
                >
                  <svg
                    width="10"
                    height="10"
            ...
            >
              {paginated.length === 0 && (
                ...
                >
                  <div
                    aria-hidden="true"
                    className={`w-11 h-11 rounded-2xl ${emptyIcon} border flex items-center justify-center`}
                  >
                    <svg
                      width="18"
                      height="18"
              ...
              {paginated.map((task) => {
                ...
                const stalled = daysBetween(
                  dates?.created ?? null,
                  dates?.started ?? null,
                );
                const inProg = daysBetween(
                  dates?.started ?? null,
                  dates?.finished ?? today(),
                );
                const isRunning = task.status === "InProgress";
                const isPending = task.status === "Pending";
                const isDone = task.status === "Done";
                const hasProrrog =
                  dates?.originalDeadline &&
                  dates?.deadline &&
                  dates.originalDeadline !== dates.deadline;
                const sla = slaInfo(
                  dates?.originalDeadline ?? null,
                  dates?.finished ?? null,
                );
                ...
                return (
                  ...
                  >
                    <div
                      className={`flex items-center gap-3 px-5 py-3.5 ${taskHover} transition group`}
                    >
                      ...
                      <div className="flex-shrink-0">
                        {isDone ? (
                          ...
                          <button
                            onClick={() => setConfirmStartId(task.id)}
                            title="Iniciar"
                      ...
                      <div className="flex-1 min-w-0">
                        {editingTitle[task.id] && !isDone ? (
                          <div className="flex items-center gap-1.5">
                            ...
                            <button
                              ...
                              className="text-[10px] px-2 py-1 rounded-md bg-violet-600 hover:bg-violet-500 text-white font-semibold transition"
                            >
                              ok
                            </button>
                            <button
                              onClick={() => cancelEditTitle(task.id)}
                              className={`text-[10px] px-2 py-1 rounded-md border ${dark ? "border-white/10 text-white/50" : "border-slate-200 text-slate-400"} transition`}
                            >
                              ✕
                            </button>
                          </div>
                          ...
                          <div className="flex items-center gap-1.5 group/title">
                            <p
                              className={`text-sm font-medium truncate transition ${isDone ? taskDoneCls : taskActiveCls}`}
                            >
                              {task.title}
                            </p>
                            {!isDone && (
                              <button
                                onClick={() =>
                                  startEditTitle(task.id, task.title)
                                }
                        ...
                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                          <span
                            className={`inline-flex items-center gap-1.5 text-[11px] font-semibold px-2 py-0.5 rounded-full ${priorityConfig[task.priority].color}`}
                          >
                            <span
                          ...
                          {dates?.deadline && (
                            <span
                              className={`text-[11px] font-medium ${dark ? "text-violet-400" : "text-violet-500"}`}
                            >
                              🎯 {fmt(dates.deadline)}
                            </span>
                          )}
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <span
                          className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-full ${statusConfig[task.status].color}`}
                        >
                        ...
                        {!savedDesc ? (
                          <button
                            onClick={() => {
                              ...
                              if (!isExpanded && !isDone) startEdit(task.id);
                            }}
                            className={`${chevronCls} transition`}
                            title="Detalhes"
                          >
                            <svg
                              width="15"
                    ...
                    {isExpanded && (
                      <div
                        className={`mx-5 mb-4 rounded-xl border ${descAreaBg} p-4 flex flex-col gap-4`}
                      >
                        {/* timeline */}
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                          {[
                            {
                              lbl: "Criada em",
                              val: fmt(dates?.created ?? null),
                              dot: "bg-slate-400",
                            },
                            {
                              lbl: hasProrrog ? "Prazo atual" : "Meta (prazo)",
                              val: fmt(dates?.deadline ?? null),
                              dot: "bg-violet-500",
                              extra: (
                                <button
                                  onClick={() => {
                            ...
                            {
                              lbl: "Concluída em",
                              val: dates?.finished ? fmt(dates.finished) : "—",
                              dot: "bg-emerald-500",
                            },
                          ].map((item) => (
                            <div key={item.lbl}>
                              <div className="flex items-center gap-1.5 mb-0.5">
                                ...
                                />
                                <span
                                  className={`text-[10px] font-semibold uppercase tracking-widest ${timelineLbl}`}
                                >
                                  {item.lbl}
                                </span>
                              </div>
                              <p
                                className={`text-xs font-medium ${timelineVal} pl-3`}
                              >
                                {item.val}
                              </p>
                              {item.extra && (
                                <div className="pl-3">{item.extra}</div>
                              )}
                            </div>
                          ))}
                        </div>
                        ...
                        {hasProrrog && (
                          <div
                            className={`-mt-1 pt-3 border-t ${dark ? "border-white/5" : "border-slate-200"} flex items-center gap-2`}
                          >
                            ...
                            <span
                              className={`text-[11px] font-medium ${dark ? "text-amber-300" : "text-amber-600"}`}
                            >
                              {fmt(dates?.originalDeadline ?? null)}
                            </span>
                            <span
                              className={`text-[10px] ${dark ? "text-white/30" : "text-slate-400"}`}
                            >
                              (prazo prorrogado)
                            </span>
                          </div>
                        ...
                        {editingDeadlineId === task.id && (
                          <div
                            className={`-mt-1 pt-3 border-t ${dark ? "border-white/5" : "border-slate-200"} flex flex-col gap-2`}
                          >
                            <div className="flex items-center gap-1.5">
                              <span
                            ...
                            <div className="pl-3 flex items-center gap-2">
                              ...
                              <button
                                ...
                                className={`text-[10px] px-1.5 py-0.5 rounded-md border ${dark ? "border-white/10 text-white/40" : "border-slate-200 text-slate-400"} transition`}
                              >
                                ✕
                              </button>
                            </div>
                        ...
                        {(deadlineHistory[task.id]?.length ?? 0) > 0 && (
                          <div
                            className={`pt-3 border-t ${dark ? "border-white/5" : "border-slate-200"} flex flex-col gap-2`}
                          >
                            <div className="flex items-center gap-1.5">
                              <span className="w-1.5 h-1.5 rounded-full bg-amber-400 flex-shrink-0" />
                              <span
                                className={`text-[10px] font-semibold uppercase tracking-widest ${timelineLbl}`}
                              >
                                Observações — alterações de prazo
                              </span>
                            </div>
                            <div className="pl-3 flex flex-col gap-2">
                              {deadlineHistory[task.id].map((entry, i) => (
                                <div
                                  key={i}
                                  className={`rounded-lg px-3 py-2 ${dark ? "bg-amber-500/5 border border-amber-500/15" : "bg-amber-50 border border-amber-200"}`}
                                >
                                  <p
                                    className={`text-[10px] ${dark ? "text-white/35" : "text-slate-400"} mb-0.5`}
                                  >
                                    Prazo inicial:{" "}
                                    <span
                                      className={`font-semibold ${dark ? "text-white/55" : "text-slate-500"}`}
                                    >
                                      {fmt(entry.oldDate)}
                                    </span>
                                  </p>
                                  <p
                                    className={`text-[10px] font-semibold ${dark ? "text-amber-400" : "text-amber-700"} mb-1`}
                                  >
                                    Novo prazo: {fmt(entry.newDate)}{" "}
                                    <span
                                      className={`font-normal ${dark ? "text-white/30" : "text-slate-400"}`}
                                    >
                                      · {entry.changedAt}
                                    </span>
                                  </p>
                                  <p
                                    className={`text-[11px] ${dark ? "text-white/60" : "text-slate-600"}`}
                                  >
                                    {entry.reason}
                                  </p>
                                </div>
                        ...
                        {/* stats de tempo */}
                        <div
                          className={`flex flex-wrap gap-4 text-xs pt-1 border-t ${dark ? "border-white/5" : "border-slate-200"}`}
                        >
                          {stalled !== null && (
                            <span className={textFaint}>
                              ⏸ Ficou parada{" "}
                              <strong className={text}>{stalled}d</strong> antes
                              de iniciar
                            </span>
                          )}
                          {isRunning && dates?.started && (
                            <span className={textFaint}>
                              ▶ Em andamento há{" "}
                              <strong className={text}>{inProg}d</strong>
                            </span>
                          )}
                          ...
                          {isPending && (
                            <span className="text-amber-500 font-medium">
                              Clique no círculo para iniciar a tarefa
                            </span>
                          )}
                        </div>
                        ...
                        {/* descrição */}
                        <div
                          className={`pt-1 border-t ${dark ? "border-white/5" : "border-slate-200"}`}
                        >
                          {isEditing && !isDone ? (
                            <>
                              <textarea
                              ...
                              <div className="flex justify-end gap-2 mt-2">
                                ...
                                <button
                                  onClick={() => saveDescToBank(task.id)}
                            ...
                            <>
                              <p
                                className={`text-sm leading-relaxed ${descTextCls}`}
                              >
                                {savedDesc}
                              </p>
                              {!isDone && (
                            ...
                            <p
                              className={`text-xs italic ${dark ? "text-white/25" : "text-slate-300"}`}
                            >
                              Sem descrição
                            </p>
                          )}
            ...
            {filtered.length > pageSize && (
              <div
                className={`flex items-center justify-between px-5 py-3 border-t ${listBdr}`}
              >
          ...
          <div className={`${cardBg} border rounded-2xl overflow-hidden`}>
            {/* cabeçalho */}
            <div
              className={`px-5 pt-4 pb-3 border-b ${dark ? "border-white/5" : "border-slate-100"}`}
            >
              ...
              <div className="flex items-center justify-between flex-wrap gap-3 mb-3">
                <div>
                  ...
                  <p className={`text-xs ${textFaint} mt-0.5`}>
                    Concluídas vs. previsto ao longo do tempo
                  </p>
                </div>
                <div className="flex items-center gap-5">
                  {[
                    ...
                    <div key={l.label} className="flex items-center gap-1.5">
                      ...
                      <span className={`text-[11px] font-medium ${textFaint}`}>
                        {l.label}
                      </span>
                    </div>
                  ))}
              ...
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    aria-live="polite"
                    aria-atomic="true"
                    className={`text-xs font-semibold ${text} min-w-[180px] text-center`}
                  >
                    {weekLabel}
                  </span>
                  <button
            ...
            <div className="sr-only">
              <table>
                <caption suppressHydrationWarning>
                  Progresso semanal — {weekLabel}
                </caption>
                <thead>
                  <tr>
            ...
            <div className="px-5 pt-5 pb-4">
              ...
              >
                <title id="chart-title" suppressHydrationWarning>
                  Gráfico de progresso — {weekLabel}
                </title>
                <desc id="chart-desc" suppressHydrationWarning>
                  Linha de tarefas concluídas, em andamento e previsto por dia
                  da semana.
                  {doneSeries[doneSeries.length - 1]} concluídas e{" "}
                  {progressSeries[progressSeries.length - 1]} em andamento no
                  último dia visível.
                </desc>
                <defs>
                  ...
                  return (
                    <g key={idx}>
                      <line
                        ...
                        y2={yg}
                        stroke={
                          dark ? "rgba(255,255,255,.055)" : "rgba(0,0,0,.06)"
                        }
                        strokeWidth="1"
                      />
                      <text
                        ...
                        fontWeight="500"
                        fill={dark ? "rgba(255,255,255,.28)" : "#94a3b8"}
                      >
                        {val}
                      </text>
                    </g>
                ...
                {chartLabels.map((_, i) => (
                  <line
                    ...
                    x2={gx(i)}
                    y2={GH - GP}
                    stroke={dark ? "rgba(255,255,255,.035)" : "rgba(0,0,0,.04)"}
                    strokeWidth="1"
                  />
                ))}
                ...
                {/* área sob concluídas */}
                <path
                  d={`${pathDone} L ${gx(nPoints - 1)} ${GH - GP} L ${gx(0)} ${GH - GP} Z`}
                  fill="url(#gDone)"
                />

                {/* área sob andamento */}
                <path
                  d={`${pathProgress} L ${gx(nPoints - 1)} ${GH - GP} L ${gx(0)} ${GH - GP} Z`}
                  fill="url(#gProgress)"
                />
                ...
                <path
                  ...
                  fill="none"
                  stroke={
                    dark ? "rgba(255,255,255,.28)" : "rgba(100,116,139,.4)"
                  }
                  strokeWidth="1.5"
                  strokeDasharray="6 3"
                  strokeLinecap="round"
                  ...
                  return (
                    <g key={i}>
                      <circle
                        ...
                        r="8"
                        fill="#10b981"
                        opacity={chartHoverIdx === i ? 0.2 : 0.1}
                      />
                ...
                {chartLabels.map((lbl, i) => {
                  const colW =
                    nPoints > 1 ? (GW - GP * 2) / (nPoints - 1) : GW - GP * 2;
                  return (
                    <g key={i}>
                      <rect
                        x={gx(i) - colW / 2}
              ...
              {/* insight */}
              <div
                className={`mt-1 pt-3 border-t ${dark ? "border-white/5" : "border-slate-100"}`}
              >
                {(() => {
                  ...
                  if (diff >= 0)
                    return (
                      <div className="flex items-center gap-2.5">
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold border ${dark ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : "bg-emerald-50 border-emerald-200 text-emerald-600"}`}
                        >
                          <svg
                            width="10"
                            height="10"
                        ...
                        <span className={`text-xs ${textFaint}`}>
                          {diff} tarefa{diff !== 1 ? "s" : ""} acima do previsto
                        </span>
                      </div>
                    );
                  return (
                    <div className="flex items-center gap-2.5">
                      <span
                        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold border ${dark ? "bg-amber-500/10 border-amber-500/20 text-amber-400" : "bg-amber-50 border-amber-200 text-amber-600"}`}
                      >
                        <svg
                          width="10"
                          height="10"
                      ...
                      <span className={`text-xs ${textFaint}`}>
                        {Math.abs(diff)} tarefa{Math.abs(diff) !== 1 ? "s" : ""}{" "}
                        abaixo do esperado
                      </span>
                    </div>
                  );
              ...
              <div className="px-5 py-5 space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[
                      {
                        label: "Total",
                        value: analytics.status.data.total_tasks,
                        accent: "text-violet-500",
                      },
                      {
                        label: "Pendente",
                        value: analytics.status.data.PENDING.count,
                        accent: dark ? "text-white/55" : "text-slate-500",
                        sub: `${analytics.status.data.PENDING.percent}%`,
                      },
                      {
                        label: "Andamento",
                        value: analytics.status.data.IN_PROGRESS.count,
                        accent: "text-blue-400",
                        sub: `${analytics.status.data.IN_PROGRESS.percent}%`,
                      },
                      {
                        label: "Concluída",
                        value: analytics.status.data.DONE.count,
                        accent: "text-emerald-400",
                        sub: `${analytics.status.data.DONE.percent}%`,
                      },
                    ].map((s) => (
                      <div
                        key={s.label}
                        className={`${dark ? "bg-white/[0.03] border-white/8" : "bg-slate-50 border-slate-200"} border rounded-xl px-4 py-3`}
                      >
                        <p
                          className={`text-[10px] font-semibold ${textFaint} uppercase tracking-widest`}
                        >
                        ...
                        <p
                          className={`text-xl font-black ${s.accent} leading-tight mt-1`}
                        >
                          {s.value}
                        </p>
                        {s.sub && (
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {[
                        {
                          label: "Alta",
                          value: analytics.priority.data.HIGH.count,
                          sub: `${analytics.priority.data.HIGH.percent}%`,
                          accent: "text-red-400",
                        },
                        {
                          label: "Média",
                          value: analytics.priority.data.MEDIUM.count,
                          sub: `${analytics.priority.data.MEDIUM.percent}%`,
                          accent: "text-amber-400",
                        },
                        {
                          label: "Baixa",
                          value: analytics.priority.data.LOW.count,
                          sub: `${analytics.priority.data.LOW.percent}%`,
                          accent: "text-emerald-400",
                        },
                      ].map((s) => (
                        <div
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        ...
                        >
                          ...
                          <p
                            className={`text-xl font-black ${text} leading-tight mt-1`}
                          >
                            {s.value}
                          </p>
                        </div>
                ...
                <div>
                  <p
                    className={`text-[10px] font-bold uppercase tracking-widest ${textFaint} mb-3`}
                  >
                    Throughput — Tarefas por Dia
                  </p>
                  {analytics.throughput?.data.length ? (
                    <div
                      className={`${dark ? "bg-white/[0.03] border-white/8" : "bg-slate-50 border-slate-200"} border rounded-xl p-4`}
                    >
                      <ResponsiveContainer width="100%" height={200}>
                        <LineChart data={analytics.throughput.data}>
                          ...
                          <XAxis
                            dataKey="day"
                            stroke={dark ? "rgba(255,255,255,.3)" : "#94a3b8"}
                            tick={{
                              fontSize: 11,
                              fill: dark ? "rgba(255,255,255,.4)" : "#64748b",
                            }}
                          />
                          <YAxis
                            stroke={dark ? "rgba(255,255,255,.3)" : "#94a3b8"}
                    ...
                    <div
                      className={`${dark ? "border-white/8 text-white/25" : "border-slate-200 text-slate-400"} border border-dashed rounded-xl px-4 py-3 text-xs`}
                    >
                      Dados insuficientes
                    </div>
                  )}
